...
const localUserDb = {
  getUsers: (): Record<string, User & { password: string }> => {
    try {
      const users = localStorage.getItem("local_users_v18")
  ...
  saveUsers: (users: Record<string, User & { password: string }>) => {
    try {
      localStorage.setItem("local_users_v18", JSON.stringify(users))
    } catch (error) {
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      try {
        setIsLoading(true)
        const storedUser = localStorage.getItem("user_v18")
        const isGuestUser = localStorage.getItem("guestUser_v18") === "true"
        ...
        if (storedUser) {
          ...
          // Si aucun utilisateur n'est trouvé, définir comme invité par défaut
          setIsGuest(true)
          const guestUser = {
            id: "guest",
            email: "guest@example.com",
        ...
        // En cas d'erreur, définir l'utilisateur comme invité
        setIsGuest(true)
        const guestUser = {
          id: "guest",
          email: "guest@example.com",
  ...
  const login = async (email: string, password: string): Promise<boolean> => {
    ...
    try {
      ...
      if (user && user.password === password) {
        ...
        localStorage.setItem("user_v18", JSON.stringify(userWithoutPassword))
        localStorage.removeItem("guestUser_v18") // S'assurer que l'état invité est supprimé
      ...
      if (email === "demo@example.com" && password === "password") {
        ...
        localStorage.setItem("user_v18", JSON.stringify(userData))
        localStorage.removeItem("guestUser_v18")
  ...
  const logout = () => {
    localStorage.removeItem("user_v18")
    localStorage.removeItem("guestUser_v18")
    setUser(null)
    setIsGuest(false)
    router.push("/v18/login-choice")
  }
