...
export default function ComparePage() {
  ...
  useEffect(() => {
    // Améliorer la gestion des erreurs dans la fonction fetchIphones

    // Remplacer la fonction fetchIphones dans useEffect par celle-ci:
    const fetchIphones = async () => {
      try {
        ...
        const response = await fetch("/v18/api/iphones", {
          // Ajouter un cache: 'no-store' pour éviter les problèmes de mise en cache
          cache: "no-store",
        })
        ...
        const phonesArray = Array.isArray(data) ? data : []
        console.log("iPhones récupérés:", phonesArray.length)

        if (phonesArray.length === 0) {
          console.warn("Aucun iPhone récupéré, utilisation des données fictives")
          // Utiliser des données fictives si aucun iPhone n'est récupéré
          const mockData = [
            {
              id: 1,
              ...
              features:
                "Dynamic Island, USB-C 3.2, Bouton Action, Design en titane, Wi-Fi 7, Bluetooth 5.3, Charge rapide 45W",
              release_year: 2024,
          ...
          setIphones(mockData)
          setSelectedPhones(mockData.slice(0, 2))
        } else {
          setIphones(phonesArray)
          // Default: select first two phones if available
          if (phonesArray.length >= 2) {
            setSelectedPhones([phonesArray[0], phonesArray[1]])
          } else if (phonesArray.length === 1) {
            setSelectedPhones([phonesArray[0]])
          }
        }

        setLoading(false)
      } catch (error) {
        console.error("Échec de la récupération des iPhones:", error)
